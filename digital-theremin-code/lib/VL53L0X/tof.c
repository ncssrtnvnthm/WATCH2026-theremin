...
// modified by Derek Woodroffe 
// Added/modified for Pico I2C calls
// Added/modified Support for multiple I2C devices
...
static int initSensor(int,uint8_t);
...
int tofInit( int i2cAddr, int bLongRange)
{
/*    //init i2c
...
int SetDeviceAddress(uint8_t iAddr,uint8_t NewIAddress){
     uint8_t ucTemp[2];
     ucTemp[0] = REG_I2C_SLAVE_DEVICE_ADDRESS;
     ucTemp[1] = NewIAddress;
     uint8_t rc=i2c_write_blocking(I2CInst, iAddr, ucTemp, 2, false);
     if(rc!=2){
...
int SetI2CAddress(uint8_t DeviceAddress){
    iAddr=DeviceAddress;
    return 0;
}
...
{
     uint8_t buffer[3];
     i2c_write_blocking(I2CInst, iAddr, &ucAddr, 1, true);
     i2c_read_blocking(I2CInst, iAddr, buffer, 2, false);
...
static unsigned char readReg(unsigned char ucAddr)
{
     uint8_t buffer;
     uint8_t rc;
     rc = i2c_write_blocking(I2CInst, iAddr, &ucAddr, 1, true);
     if (rc == 1){
          rc = i2c_read_blocking(I2CInst, iAddr, &buffer, 1, false);
	  if (rc != 1) {};
     }
     return buffer;
} /* ReadReg() */
...
static void readMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
     ...
     rc = i2c_write_blocking(I2CInst, iAddr, &ucAddr, 1, true);
     if (rc == 1){
          rc = i2c_read_blocking(I2CInst, iAddr, pBuf, iCount, false);
          if (rc != iCount) {};
     }
} /* readMulti() */
...
static void writeMulti(unsigned char ucAddr, unsigned char *pBuf, int iCount)
{
     uint8_t rc;
     i2c_write_blocking(I2CInst, iAddr, &ucAddr, 1, true);
     rc=i2c_write_blocking(I2CInst, iAddr, pBuf, iCount, false);        
     if (rc != iCount) {};
} /* writeMulti() */
//
...
int rc;
     ...
     rc=i2c_write_blocking(I2CInst, iAddr, ucTemp, 3, false);
...
int rc;
     ...
     ucTemp[0] = ucAddr;
     ucTemp[1] = ucValue;
     rc=i2c_write_blocking(I2CInst, iAddr, ucTemp, 2, false);
...
unsigned char ucCount = *ucList++; // count is the first element in the list
    ...
    while (ucCount){
        rc=i2c_write_blocking(I2CInst, iAddr,ucList, 2, false);
        if (rc != 2) {};
            ucList += 2;
            ucCount--;
	}
...
//
static int initSensor(int bLongRangeMode,uint8_t iA)
{
    iAddr=iA;
...
uint16_t readRangeContinuousMillimeters(uint8_t iA)
{
    iAddr=iA;
    int iTimeout = 0;
    uint16_t range;
...
//
int tofReadDistance(uint8_t iA)
{
    iAddr=iA;
    int iTimeout;
    ...
    return readRangeContinuousMillimeters(iA);

} /* tofReadDistance() */

int tofGetModel(int *model, int *revision, uint8_t iA)
{
    iAddr=iA;
    unsigned char ucTemp[2];
    int i;
    ...
    if (model)
    {
        ...
        i2c_write_blocking(I2CInst, iAddr, ucTemp, 1, true);        	
        i=i2c_read_blocking(I2CInst, iAddr, ucTemp, 1, false);        	
        if (i == 1) *model = ucTemp[0];
    }
    if (revision)
    {
